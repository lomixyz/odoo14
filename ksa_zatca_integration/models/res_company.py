...
class ResCompany(models.Model):
    ...
    def constrains_brksa64(self):
        for record in self:
            ...
            if record.is_zatca:
                # BR-KSA-37
                if len(str(self.sanitize_int(record.building_no))) != 4:
                    raise exceptions.ValidationError(_('Building Number must be exactly 4 digits'))
                # BR-KSA-66
                if len(str(self.sanitize_int(record.zip))) != 5:
    ...
    def generate_zatca_certificate(self, auto_compliance=0):
        ...
        try:
            ...
            conf.csr_serial_number = ("1-Odoo|2-14|3-%s_%s_%s" % (odoo.release.version.replace('14.0', '').replace("+e", "e").replace("-", ""), self.id, str(uuid.uuid4()).replace("-", ""))).encode('utf-8')
