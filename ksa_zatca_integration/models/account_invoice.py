...
class AccountMove(models.Model):
    ...
    # amount_tax_signed = fields.Monetary(string='Tax', store=True, readonly=True, compute='_compute_amount', currency_field='company_currency_id')
    ...
    #             amount_tax_signed = move.currency_id._convert(
    #                 move.amount_tax, move.company_id.currency_id, move.company_id, move.invoice_date or fields.Date.today()
    #             )
    #             move.amount_tax_signed = amount_tax_signed
    #         sign = move.move_type in ['in_refund', 'out_refund'] and -1 or 1
    #         move.amount_tax_signed = move.amount_tax_signed * sign
    # @api.one
    # @api.depends('invoice_line_ids.price_subtotal',
    #              'currency_id', 'company_id', 'invoice_date', 'type')
    # def _compute_amount(self):
    #     super(AccountMove, self)._compute_amount()
    #     self.amount_tax_signed = self.amount_tax
    #     if self.currency_id and self.company_id and self.currency_id != self.company_id.currency_id:
    #         currency_id = self.currency_id.with_context(date=self.invoice_date)
    #         self.amount_tax_signed = currency_id.compute(self.amount_tax, self.company_id.currency_id)
    #     sign = self.move_type in ['in_refund', 'out_refund'] and -1 or 1
    #     self.amount_tax_signed = self.amount_tax_signed * sign

    def _default_l10n_sa_invoice_type_is_readonly(self):
        return 1 if self.env.user.company_id.sudo().zatca_invoice_type != "Standard & Simplified" else 0
    ...
    sa_confirmation_datetime = fields.Datetime(string='Confirmation Date', readonly=True, copy=False)
    ...
    def _prepare_refund(self, invoice, invoice_date=None, date=None, description=None, journal_id=None):
        res = super()._prepare_refund(invoice, invoice_date=invoice_date, date=date, description=description, journal_id=journal_id)
        res['l10n_sa_invoice_type'] = self.l10n_sa_invoice_type
        res['l10n_is_self_billed_invoice'] = self.l10n_is_self_billed_invoice
        return res

    @api.model
    def _prepare_debitnote(self, invoice, invoice_date=None, date=None, description=None, journal_id=None):
        res = super()._prepare_debitnote(invoice, invoice_date=invoice_date, date=date, description=description, journal_id=journal_id)
        res['l10n_sa_invoice_type'] = self.l10n_sa_invoice_type
        res['l10n_is_self_billed_invoice'] = self.l10n_is_self_billed_invoice
        return res

    @api.depends('zatca_compliance_invoices_api', 'sa_confirmation_datetime')
    def _compute_l10n_sa_zatca_status(self):
        for res in self:
            res.l10n_sa_zatca_status = "Not Sended to Zatca"
            sa_confirmation_datetime = datetime.datetime.strptime(str(res.sa_confirmation_datetime), DEFAULT_SERVER_DATETIME_FORMAT) if res.sa_confirmation_datetime else False
            l10n_sa_phase1_end_date = datetime.datetime.strptime(str(res.l10n_sa_phase1_end_date), DEFAULT_SERVER_DATE_FORMAT).date() if res.l10n_sa_phase1_end_date else False
            if sa_confirmation_datetime and l10n_sa_phase1_end_date and sa_confirmation_datetime.date() <= l10n_sa_phase1_end_date:
    ...
    def get_signature(self, conf=0):
        ...
        _zatca.info("conf.csr_certificate:: %s", conf.csr_certificate)
        if not certificate:
        ...
        _zatca.info("certificate:: %s", certificate)
        ...
        try:
            ...
            _zatca.info("cert.issuer:: %s", cert.issuer)
    ...
    def tax_invoice_validations(self):
        ...
        if conf.csr_invoice_type[0:1] != '1':
            raise exceptions.AccessError(_("Certificate not allowed for Standard Invoices."))
    ...
    def create_xml_file(self, previous_hash=0, pos_refunded_order_id=0):
        ...
        conf = self.company_id.sudo()
        if not self.sa_confirmation_datetime:
            if not self.l10n_sa_show_delivery_date:
                raise exceptions.UserError(_('Delivery Date cannot be empty'))
                self.sa_confirmation_datetime = fields.Datetime.now()
            self.write({'sa_confirmation_datetime': fields.Datetime.now()})
        ...
        if not conf.is_zatca:
            raise exceptions.AccessError(_("Zatca is not activated."))
        # No longer needed
        ...
        if not is_tax_invoice and conf.csr_invoice_type[1:2] != '1':
            raise exceptions.AccessError(_("Certificate not allowed for Simplified Invoices."))
        ...
        sa_confirmation_datetime = datetime.datetime.strptime(str(self.sa_confirmation_datetime), DEFAULT_SERVER_DATETIME_FORMAT)
        ubl_2_1 += '''
            <cbc:ProfileID>reporting:1.0</cbc:ProfileID>
            <cbc:ID>''' + str(self.check_allowed_size(1, 127, bt_1, 'bt_1')) + '''</cbc:ID>
            <cbc:UUID>''' + self.invoice_uuid + '''</cbc:UUID>
            <cbc:IssueDate>''' + self.sa_confirmation_datetime.strftime('%Y-%m-%d') + '''</cbc:IssueDate>
            <cbc:IssueTime>''' + self.sa_confirmation_datetime.strftime('%H:%M:%SZ') + '''</cbc:IssueTime>
        ...
        if bt_3 != '388':  # BR-KSA-56
            bt_25_sa_confirmation_datetime = datetime.datetime.strptime(str(bt_25.sa_confirmation_datetime or fields.Datetime.now()), DEFAULT_SERVER_DATETIME_FORMAT)
            ubl_2_1 += '''
            <cac:BillingReference>
                <cac:InvoiceDocumentReference>
                    <cbc:ID>''' + str(self.check_allowed_size(1, 5000, bt_25.id, 'bt_25')) + '''</cbc:ID>
                    <cbc:IssueDate>''' + str(bt_25_sa_confirmation_datetime.strftime('%Y-%m-%d')) + '''</cbc:IssueDate>
                </cac:InvoiceDocumentReference>
            </cac:BillingReference>'''
        ubl_2_1 += '''
        ...
        # bt_121 = list(set(self.invoice_line_ids.mapped('tax_ids.tax_exemption_selection')))
        ...
        if delivery and ((bt_3 == '388' and ksa_2[:2] == '01' or not is_tax_invoice and self.l10n_is_summary_invoice) or (latest_delivery_date and not_know)):
            ubl_2_1 += '''
            <cac:Delivery>'''
            ksa_5 = datetime.datetime.strptime(str(l10n_sa_delivery_date), DEFAULT_SERVER_DATE_FORMAT)
            ...
            if latest_delivery_date and not_know:
                ksa_24 = datetime.datetime.strptime(str(l10n_sa_delivery_date) , DEFAULT_SERVER_DATE_FORMAT)
                if ksa_24 < ksa_5:
        ...
        if bt_3 != '388':
            ubl_2_1 += '''
            <cbc:InstructionNote>''' + self.check_allowed_size(1, 1000, self.name, self._fields['name'].string) + '''</cbc:InstructionNote>'''
        ubl_2_1 += '''
        </cac:PaymentMeans>'''
        ...
        for invoice_line_id in self.invoice_line_ids:
            ...
            def next_invoice_line_id(invoice_line_id):
                id = self.env['ir.sequence'].with_context(company_id=self.company_id.id).next_by_code('zatca.move.line.seq')
        ...
        file_name_specification = str(bt_31) + "_" + sa_confirmation_datetime.strftime('%Y%m%dT%H%M%SZ') + "_" + str(self.id)
    ...
    def compliance_invoices_api(self, auto_compliance=0, **kwargs):
        ...
        if not auto_compliance:
            ...
            zatca_invoice = atts.datas.decode('UTF-8')
        else:
            zatca_invoice = kwargs.get('zatca_invoice')
    ...
    def invoices_clearance_single_api(self):
        ...
        data = {
            ...
            'invoice': atts.datas.decode('UTF-8'),
        }
        try:
            req = requests.post(link + endpoint, headers=headers, data=json.dumps(data), timeout=(30, 60))
            self.l10n_sa_response_datetime = fields.Datetime.now()
            if req.status_code == 500:
                ...
                sa_confirmation_datetime = datetime.datetime.strptime(str(self.sa_confirmation_datetime), DEFAULT_SERVER_DATETIME_FORMAT)
                file_name_specification = str(company_id.vat) + "_" + sa_confirmation_datetime.strftime('%Y%m%dT%H%M%SZ') + "_" + str(re.sub(r"[^a-zA-Z0-9]", "-", self.zatca_unique_seq))
    ...
    def hash_with_c14n_canonicalization(self, conf, api_invoice=0, xml=0, auto_compliance=0):
        ...
        try:
            _zatca.info(invoice)
            _zatca.info(self.zatca_invoice)
            xml_file = ET.fromstring(invoice)
            _zatca.info(xml_file)
        except Exception as e:
    ...
    def _compute_qr_code_str(self):
        ...
        self = self.sudo()
        try:
            invoice_date = datetime.datetime.strptime(str(self.invoice_date), DEFAULT_SERVER_DATE_FORMAT).date()
            l10n_sa_phase1_end_date = datetime.datetime.strptime(str(self.l10n_sa_phase1_end_date), DEFAULT_SERVER_DATE_FORMAT).date() if self.l10n_sa_phase1_end_date else False
            if not self.is_zatca or (phase_1_ending_date and invoice_date <= phase_1_ending_date):
    ...
    def compute_qr_code_str(self, signature_value, is_tax_invoice, bt_112, bt_110, auto_compliance=0):
        ...
        try:
            for record in self:
                if record.country_code == 'SA' and record.move_type in ('out_invoice', 'out_refund') and not record.sa_confirmation_datetime:
                    if not record.l10n_sa_show_delivery_date:
                        raise exceptions.UserError(_('Delivery Date cannot be empty'))
                    record.write({'sa_confirmation_datetime': fields.Datetime.now()})
                qr_code_str = ''
                conf_company = self._get_zatca_partner_data() if self.l10n_is_self_billed_invoice else self._get_zatca_company_data(self.company_id)
                ...
                if record.sa_confirmation_datetime and record.company_id.vat:
                    ...
                    sa_confirmation_datetime = datetime.datetime.strptime(str(record.sa_confirmation_datetime), DEFAULT_SERVER_DATETIME_FORMAT)
                    time_sa = fields.Datetime.context_timestamp(self.with_context(tz='Asia/Riyadh'), sa_confirmation_datetime)
                    timestamp_enc = get_qr_encoding(3, sa_confirmation_datetime.strftime('%Y-%m-%dT%H:%M:%SZ'))
    ...
    def send_multiple_to_zatca(self):
        ...
        # if len(self.ids) and int(self[0].zatca_icv_counter) > 1:
                    ...
                    if not record.zatca_invoice_name or not record.zatca_compliance_invoices_api or \
                        ...
                        if not record.zatca_onboarding_status:
                            ...
                            if record.l10n_sa_invoice_type == 'Standard':
                                ...
                                record.send_for_reporting(self._context)
            except Exception as e:
                # Bypass errors.
        ...
        for record in self:
            ...
            if (conf.is_zatca
                ...
                if (record.move_type in ['in_invoice', 'in_refund'] and record.l10n_is_self_billed_invoice) or record.move_type in ['out_invoice', 'out_refund']:
                    ...
                    if record.env.user.company_id.zatca_send_from_pos:
                        if not record.zatca_onboarding_status:
                            record.send_for_compliance()
    ...
    def action_post(self):
        ...
        if self.env.user.company_id.zatca_send_from_pos:
        ...
        if self.delivery_count == 0 and self.move_type in ['out_invoice']:
    ...
    def _compute_show_delivery_date(self):
        for move in self:
            ...
            move.l10n_sa_show_delivery_date = (move.country_code == 'SA' and
    ...
    def unlink(self):
        for record in self:
            if record.l10n_sa_prohibited_exception() and record.state != 'draft':
                raise exceptions.AccessError(_(message))
