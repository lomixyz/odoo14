        ...
        for stage in company_id.dashboard_filter:
            ...
            if int(kw.get('team')) != 0:
                ...
                if request.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_team_leader') and request.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_user') and not request.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_manager'):
                    team_ids = request.env['helpdesk.team'].sudo().search(
                        ['|', ('team_head', '=', request.env.user.id), ('member_ids', 'in', [request.env.user.id])])
            ...
            if int(kw.get('team_leader')) != 0:
                ...
                if request.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_team_leader') and request.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_user') and not request.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_manager'):
                    ...
                    doman.append(('domain_user_ids', 'in', [request.env.user.id]))
            if int(kw.get('user_id')) != 0:
                doman.append(('|'))
                doman.append(('user_id', '=', int(kw.get('user_id'))))
                doman.append(('domain_user_ids', 'in', [int(kw.get('user_id'))]))
            ...
            search_stage = request.env['helpdesk.stage'].sudo().search([
                ('id', '=', stage.id)
            ], limit=1)
            if search_stage:
            ...
            if search_tickets:
                for ticket in search_tickets:
                    ...
                    ticket_dic = {
                        'ticket_id': ticket.id,
                        'ticket_no': ticket.name,
                        'partner_name':  ticket.partner_id.name, #ticket.partner_id.name_get()[0][1],
