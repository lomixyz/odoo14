...
class MassUpdateWizard(models.TransientModel):
    ...
    assigning = fields.Many2one(comodel_name='res.users',
                                string='Assign To',
                                domain=[('share', '=', False)])
    ...
    helpdesk_stages = fields.Many2one('helpdesk.stage', string="Stage ")
    ...
    def update_record(self):
        ...
        if self.check_assign_to == True:
            self.helpdesks_ticket_ids.write({'user_id': self.assigning.id})
        ...
        if self.check_assign_to_multiuser == True:
            ...
            if self.ticket_update_type == 'add':
                get_list = []
                for rec in self.helpdesks_ticket_ids.domain_user_ids:
                    if rec:
                ...
                self.helpdesks_ticket_ids.write(
                    {'domain_user_ids': [(6, 0, get_list)]})

            if self.ticket_update_type == "replace":
                self.helpdesks_ticket_ids.write(
                    {'domain_user_ids': [(6, 0, self.assign_to_multiuser.ids)]})
