...
class FreightOperation(models.Model):
    ...
    customer_direction = fields.Char(string="Customer Direction")
    ...
    clearance_no = fields.Char(string='Customs Clearance Number')
    saber = fields.Char(string='Saber')
    ...
    vendor_id = fields.Many2one('res.partner', string='Supplier Company')
    vendor_bill = fields.Char(string='Supplier Bill')
    ...
    state = fields.Selection([('draft', 'New'),
                              ...
                              ('delivered', 'Delivered'),
                              ('cancel', 'Cancel')],
                             default="draft", group_expand="_expand_states", string="Status")
    ...
    opp_party_id = fields.Many2many('res.partner', string="Carrier")
    customs_no = fields.Char(string='Customs declaration number')
    customs_date = fields.Date(string='Customs declaration Date')
    ...
    commission_ids = fields.One2many('freight.commission', 'freight_id')
    tracking_status_ids = fields.One2many('tracking.status', 'freight_id')
    total_amount_commission = fields.Float(string="Total Amount", readonly=True, compute='_compute_commission_amount')
    total_cartons = fields.Integer(string="Total Cartons", readonly=True, compute='_compute_cartons')
    total_package = fields.Integer(string="Total Package", readonly=True, compute='_compute_cartons')
    rating_template_id = fields.Many2one('mail.template', string='Rating Email Template',
                                         domain=[('model', '=', 'freight.operation')],
                                         )

    def _expand_states(self, states, domain, order):
        print("*********************************************", [key for key, val in type(self).state.selection])
    ...
    #     for record in self:
    #         record.write({
    #             'tracking_status_ids': [(0, 0, {'name': record.state,
    ...
    def _compute_cartons(self):
        for com in self:
            total_cartons = 0
            total_package = 0
            for line in com.operation_line_ids:
                total_package += line.package
                total_cartons += line.cartons
            com.total_cartons = total_cartons
            com.total_package = total_package

    @api.depends('commission_ids')
    def _compute_commission_amount(self):
        for com in self:
            total_amount = 0.00
            for line in com.commission_ids:
                total_amount += line.amount
            com.total_amount_commission = total_amount
    ...
    @api.constrains('order_date', 'exp_send_date', 'act_send_date', 'exp_rec_date', 'act_rec_date')
    ...
    def action_ordering(self):
        ...
        for operation in self:
            ...
            operation.write({
                'tracking_status_ids': [(0, 0, {'name': dict(self._fields['state'].selection).get(operation.state),
                                                'date': date.today().strftime('%Y-%m-%d'),
                                                'user_id': operation.env.uid,
                                                'freight_id': operation.id})],
            })
    ...
    def action_in_confirm(self):
        """Send operation in Confirm state."""
        for operation in self:
            operation.write({'state': 'confirm'})
            operation.write({
                'tracking_status_ids': [(0, 0, {'name': dict(self._fields['state'].selection).get(operation.state),
                                                'date': date.today().strftime('%Y-%m-%d'),
                                                'user_id': operation.env.uid,
    ...
    def send_task_rating_mail(self, force_send=False):
        for rec in self:
            rating_template = rec.rating_template_id
            if rating_template:
                rec.rating_send_request(rating_template, lang=rec.customer_id.lang, force_send=force_send)

            rec.write({
                'tracking_status_ids': [(0, 0, {'name': 'Send Rating Mail',
                                                'date': date.today().strftime('%Y-%m-%d'),
                                                'user_id': rec.env.uid,
                                                'freight_id': rec.id})],
            })
    ...
    def rating_get_partner_id(self):
        res = super(FreightOperation, self).rating_get_partner_id()
        if not res and self.customer_id:
            return self.customer_id
        return res

    def rating_apply(self, rate, token=None, feedback=None, subtype_xmlid=None):
        return super(FreightOperation, self).rating_apply(rate, token=token, feedback=feedback,
                                                          subtype_xmlid="scs_freight.mt_freight_rating")
...
class FreightOperationLine(models.Model):
    ...
    @api.depends('billing_on', 'price_list_id',
                 'exp_gross_weight', 'exp_vol', 'price')
    def _compute_calculate_sale_price(self):
        for line in self:
            ...
            elif line.billing_on == 'price':
                line.sale_price = line.price
...
class FreightCommission(models.Model):
    """Freight Commission Model."""

    _name = 'freight.commission'
    _description = 'Freight Commission'

    name = fields.Char('Description')
    date = fields.Date(default=fields.Date.context_today)
    amount = fields.Float('Amount')
    freight_id = fields.Many2one('freight.operation')
    employee_id = fields.Many2one('hr.employee', 'Employee')


class TrackingStatus(models.Model):
    _name = 'tracking.status'
    _description = 'Tracking Status'
