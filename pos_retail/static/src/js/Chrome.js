    ...
    const RetailChrome = (Chrome) =>
        class extends Chrome {
            ...
            async openApplication() {
                let {confirmed, payload: result} = await this.showPopup('ConfirmPopup', {
                    title: 'Welcome to POS Retail. 1st POS Solution of WAY',
            ...
            async showAppInformation() {
                let {confirmed, payload: result} = await this.showPopup('ConfirmPopup', {
                    title: 'Thanks for choice POS Retail. 1st POS Solution of WAY',
                    body: 'Copyright (c) 2014-2020 of TL TECHNOLOGY \n' +
                        '  Email: thanhchatvn@gmail.com \n' +
                        '  Skype: thanhchatvn \n' +
            ...
            async _closePos() {
                ...
                if (!pingServer) {
                    await this.showPopup('OfflineErrorPopup', {
                        ...
                        body: this.env._t('Your Internet or WAY Server Offline. If you close a POS, could not open back'),
                    });
                    return true;
                }
                const self = this;
                ...
                if (this.env.pos.user && this.env.pos.config.allow_closing_session) {
                    ...
                    lists.push({
                        name: this.env._t('Close POS Screen, Closing Posting Entries and Logout WAY'),
                        item: 3,
                        id: 3,
                    })
                    lists.push({
