...
odoo.define('pos_retail.load_models', function (require) {
    ...
    exports.posSyncBackend = Backbone.Model.extend({
        ...
        start: function () {
            ...
            this.products_wait_update = []
            this.partners_wait_update = []
        ...
        async _automaticUpdate() {
            console.warn('_automaticUpdate() products/partners - ' + this.products_wait_update.length + '/' + this.partners_wait_update.length)
            if (this.partners_wait_update.length) {
                let partnersSync = this.pos.getDatasByModel('res.partner', [['id', 'in', this.partners_wait_update]])
                if (partnersSync.length) {
                    this.pos.indexed_db.write('res.partner', partnersSync);
                    this.pos.save_results('res.partner', partnersSync);
                }
                for (let i = 0; i < this.partners_wait_update.length; i++) {
                    let partner_id = this.partners_wait_update[i]
            ...
            if (this.products_wait_update.length) {
                let productsMissed = this.pos.getDatasByModel('product.product', [['id', 'in', this.products_wait_update], ['sale_ok', '=', true], ['available_in_pos', '=', true]])
                ...
                for (let i = 0; i < this.products_wait_update.length; i++) {
                    let product_id = this.products_wait_update[i]
                    let product = this.pos.db.get_product_by_id(product_id);
                    if (product) {
                        posbus.trigger('reload.product.item', product_id)
                    }
                }
                this.pos.set('productsModifiers', this.products_wait_update)
                this.products_wait_update = []
            }
            setTimeout(_.bind(this._automaticUpdate, this), 5000);
        },
                ...
                for (let i = 0; i < notifications.length; i++) {
                    ...
                    if (channel == 'sync.backend') {
                        ...
                        for (let i = 0; i < record_ids.length; i++) {
                            ...
                            if (model == 'res.partner' && !this.partners_wait_update.includes(record_id)) {
                                this.partners_wait_update.push(record_id)
                            }
                            if (model == 'product.product' && !this.products_wait_update.includes(record_id)) {
                                this.products_wait_update.push(record_id)
                            }
                        }
                    }
                }
    ...
    let extend_models = [
        ...
        },
        {
            ...
            loaded: function (self, partner_tags) {
                ...
                self.partner_title_tag_by_id = {};
                for (let i = 0; i < partner_tags.length; i++) {
                    let title = partner_tags[i];
                    self.partner_title_tag_by_id[title.id] = title;
                }
            }
        },
        {
