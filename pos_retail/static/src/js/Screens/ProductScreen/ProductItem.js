        ...
        class extends ProductItem {
            ...
            async _autoSyncBackend() {
                if (this.env.pos.offlineModel) {
                    console.warn('You network of Odoo offline')
                }
                if (!this.env.pos.config.sync_products_realtime || this.state.refresh == 'connecting' || this.env.pos.offlineModel) {
                    return true
                }
                const self = this
                this.env.pos.set_synch('connecting', '')
                this.state.refresh = 'connecting'
                const object = this.env.pos.get_model('product.product');
                const products = await this.rpc({
                    ...
                    if (error && error.message && error.message.code == -32098) {
                        self.env.pos.set_synch('disconnected', 'Odoo Offline')
                    } else {
                ...
                if (products != null) {
                    console.log('sync product id: ' + this.props.product.id)
