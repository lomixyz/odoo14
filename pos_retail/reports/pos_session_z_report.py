...
class pos_session(models.Model):
    ...
    def build_sessions_report(self):
        ...
        for session in self:
            ...
            session_report['payment_partial_amount'] = session.get_parial_payments_amount()
    ...
    def get_sale_value(self):
        ...
        sale_obj = self.env['sale.order'].sudo().search([
                ('pos_order_id', '=', self.ids)
            ])
        print('////////////////////',sale_obj)

        line = sale_obj.read_group([], fields=['payment_partial_method_id', 'payment_partial_amount'],
                                  groupby=['payment_partial_method_id', 'payment_partial_amount'], lazy=False)
        values.append(line)
        print('////////////////????????????/////', values)
        print(type(values))
        if len(values) > 0:
            print(str(values))  # << DEBUG breakpoint here
            return values
        # #     return {
        # #         'payment_partial_method_id': values[0]['payment_partial_method_id'],
        # #         'payment_partial_amount': values[0]['payment_partial_amount'],
        # #     }
        else:
            return None
        ...
        # sale_obj = self.env['sale.order'].read_group([
        #     'payment_partial_method_id',
        #     'payment_partial_amount'
        # ],
        # ['payment_partial_method_id'])
        # print("tessssst",sale_obj)
        # sale_line = sale_obj.search([])
    ...
    def get_parial_payments_amount_sale_adv(self):
        payments_amount = []
        journal_ids = self.env['account.journal'].search([("type", "in", ("bank", "cash"))])
        sale_order = self.env['sale.order'].search([
                ('date_order', '>=', self.start_at),
                ('date_order', '<=', self.stop_at or fields.datetime.now()),
                ('pos_config_id', '=', self.config_id.id),
                # ('payment_partial_method_id', '=', payment_method.id)
            ])
        print("***************************", self.start_at)
        print("***************************", self.stop_at)
        print("***************************", sale_order)
        for payment_method in journal_ids:
            sale_dict = {
                'name': payment_method.name,
                'amount': 0
            }
            payment_ids = sale_order.account_payment_ids.filtered(lambda line: line.journal_id == payment_method)
            for payment in payment_ids:
                amount = payment.amount
                sale_dict['amount'] += amount
            if sale_dict['amount'] > 0 :
                payments_amount.append(sale_dict)
                print("(((((((((((((((&&&&&&&&&&&&&&&&",sale_dict['amount'])
        return payments_amount
    
    def get_parial_payments_amount(self):
        payments_amount = []
        for payment_method in self.config_id.payment_method_ids:
            payments = self.env['sale.order'].search([
                ('date_order', '>=', self.start_at),
                ('date_order', '<=', self.stop_at),
                ('pos_config_id', '=', self.config_id.id),
                ('payment_partial_method_id', '=', payment_method.id)
            ])
            sale_dict = {
            ...
            print("333333333333333333333",str(payments))
            for payment in payments:
                amount = payment.payment_partial_amount
                sale_dict['amount'] += amount
            payments_amount.append(sale_dict)
        return payments_amount
