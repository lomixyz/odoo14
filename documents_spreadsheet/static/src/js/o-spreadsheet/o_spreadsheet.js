(function (exports, owl) {
    ...
    function formatJSTime(date, format) {
        ...
        return (parts
            ...
            switch (p) {
                case "hhhh":
                    const helapsedHours = Math.floor((date.getTime() - INITIAL_1900_DAY$1) / (60 * 60 * 1000));
  ...
  .o-cf {
    ...
    function compile(str, sheet, sheets) {
        ...
        function compileFunctionArgs(ast) {
            const fn = functions$1[ast.value.toUpperCase()];
            const result = [];
            const args = ast.args;
            let argDescr;
            ...
            if (args.length < minArg || args.length > maxArg) {
                throw new Error(_lt("Invalid number of arguments for the %s function. Expected %s, but got %s instead.", ast.value.toUpperCase(), fn.args.length.toString(), args.length.toString()));
            }
            for (let i = 0; i < args.length; i++) {
                ...
                if (["BOOLEAN", "NUMBER", "STRING"].includes(arg.type)) {
                    const isRangeOnly = types.every((t) => t === "RANGE" ||
                        ...
                        t === "RANGE<STRING>");
                    if (isRangeOnly) {
                        throw new Error(_lt("Function %s expects the parameter %s to be reference to a cell or range, not a %s.", ast.value.toUpperCase(), (i + 1).toString(), arg.type.toLowerCase()));
                    }
                }
                else if (
...
`;
    ...
    exports.__info__.date = '2021-11-29T08:42:42.852Z';
    exports.__info__.hash = 'f5c7d3a';
