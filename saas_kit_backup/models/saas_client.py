...
class SaasClientBackup(models.Model):
    ...
    def get_backup(self):
        ...
        res = self.backup_process_id.call_backup_script(master_pass=config_dict['container_master'], port_number=self.containter_port, url='localhost:'+self.containter_port, db_user=db_user, db_password=db_password)
        if res.get('success'):
    ...
    def create_backup_process(self, frequency=1, frequency_cycle=None, backup_starting_time=None):
        ...
        """
        user_time_zone = self.saas_contract_id.partner_id.tz
