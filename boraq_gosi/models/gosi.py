...
class Gosi(models.Model):
    ...
    birthday = fields.Date('Date of Birth', groups="hr.group_hr_user", required=True, default= lambda self: fields.Date.context_today(self))
    ...
    def compute_age(self):
        for res in self:
            res.age = math.floor(float((datetime.date.today() - res.birthday).days / 365.25))
            if 18 <= res.age <= 60:
                res.limit = True
            else:
                res.limit = False
